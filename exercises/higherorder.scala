object HigherOrderFunctions {
    ...
    def combine[A, B, C](first: (A => B), second: (B => C)):(A => C) = (A) => second(first(A))
    ...
    /** converts a two-argument integer-taking function to a single-argument function
      ...
      */
    def curry2int(fn: (Int, Int) => Int, param: Int): (Int => Int) = (a:Int) => fn(param, a)
    ...
    /** 'curries' a two-argument generic function - i.e. translates the evaluation of a function 
      ...
      */
    def curry2[A, B, C](fn: (A, B) => C): (A => (B => C)) = (A) => (B) => fn(A, B)
