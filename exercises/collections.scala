object Functions {
    ...
    def listOfAllCharacters(list:List[String]):List[Char] = list.reduce((y,x) => y + x).toList


    // return the first string in a list, or if list is empty, return "nada"
    def firstElementInListOrNada(list:List[String]) = if(list.size > 0) list.head else "nada"

    // find the longest string in given list of strings, or "nada" if empty
    // and reverse the result before returning it
    def longestNameInList(names:List[String]): String = if(names.isEmpty) "nada".reverse else (names.reduce((x,y) => if (x.length > y.length) x.reverse else y))

    // Join list `xs` of string with separator string `sep` into a string
    def joinWithSep(xs: List[String], sep: String): String = xs.reduce((x, y) => x + sep + y)

    // Take sums of lists and return those with sum greater than 20
    //
    // sumOfLists20(List(List(1,2,3), List(2,10,9))) == List(21)
    def sumOfLists20(xs: List[List[Int]]): List[Int] = xs.map((x) => x.sum).filter((x) => x > 20)
